...
module FreeFall (F : Frame) =
  struct
    ...
    fun ((x0, y0), (dx0, dy0)) ->
      ...
      let vit = Flux.map (fun (dx, dy) -> (dx +. dx0, dy +. dy0)) (integre F.dt acc) in
      let pos = Flux.map (fun (x, y) -> (x +. x0, y +. y0)) (integre F.dt vit) in
      Flux.map2 (fun x y -> x, y) pos vit
  end

module Bouncing (F: Frame) =
struct
  let rec unless : 'a Flux.t-> ('a -> bool) -> ('a -> 'a Flux.t) -> 'a Flux.t =
  fun flux cond f_flux -> Flux.(match uncons flux with
    | None -> vide
    | Some (t, q) -> if cond t then f_flux t else Tick (lazy (Some (t, unless q cond f_flux)))
  )

  let contact_x : float -> float -> bool =
  fun x dx -> F.(x < fst box_x && (x +. dx) < fst box_x) || F.(x > snd box_x && (x +. dx) > snd box_x)

  let rebond_x : float -> float -> float =
  fun x dx -> if contact_x x dx then -.dx else dx

  let contact_y : float -> float -> bool =
  fun y dy -> F.(y < fst box_y && (y +. dy) < fst box_y) || F.(y > snd box_y && (y +. dy) > snd box_y)
...
module FF = FreeFall (Box)
module B = Bouncing (Box)
module Sim = Drawing (Box)

let sim =
  let init = ((100., 800.), (50., 50.)) in
  Sim.draw (B.run init)